...
| Project | What it is |
|:---|:---|
| **[AI-Enabled-Regression-Test-Optimization](AI-Enabled-Regression-Test-Optimization)** | An AI planner decides which regression tests a change actually needs, and pytest runs them end to end on real hardware rather than in a simulator. It reads what changed in the build and what state the device is in, chooses the smallest sufficient set of tests and the stress they run under, and records why it chose them, so the selection is repeatable rather than one engineer's judgement on the day. Two interchangeable model-provider adapters, a Zephyr firmware target on nRF52840, and a Windows binary come with it. |

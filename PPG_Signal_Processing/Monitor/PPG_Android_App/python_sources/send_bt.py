...
"""
send_bt.py -- (LINUX) stream a PPG recording to the Android PPG Monitor over
Bluetooth Classic RFCOMM (SPP).

The phone runs an RFCOMM server; this looks up its SPP channel over SDP,
connects, and sends one integer sample per line, paced at the sample rate.

Prereqs:
    sudo apt-get install -y libbluetooth-dev
    pip install pybluez2
    (pair the phone once with bluetoothctl: pair / trust the phone's MAC)

Usage:
    python3 send_bt.py <PHONE_BT_MAC> <record_file> [rate_hz] --subject TYPE
Example:
    python3 send_bt.py AA:BB:CC:DD:EE:FF synthesized_sample_adult.txt 125 \
        --subject adult
    python3 send_bt.py AA:BB:CC:DD:EE:FF synthesized_sample_neonate.txt 125 \
        --subject neonate
...
def main() -> int:
    ...
    print(f"looking up SPP service on {phone_mac} ...")
    services = bluetooth.find_service(uuid=SPP_UUID, address=phone_mac)
    if not services:
        print("No SPP service found. Is the app open and showing "
              "'Waiting for laptop to connect', and are the devices paired?")
        return 1

    # The phone advertises SEVERAL SPP services (OnePlus system ones share the
    # generic SPP UUID). Pick OUR app's record by its service name.
    print(f"found {len(services)} SPP service(s):")
    for s in services:
        print(f"  name={s.get('name')!r}  channel={s.get('port')}")
    mine = [s for s in services if (s.get("name") or "") == "PPGMonitor"]
    if not mine:
        print("PPGMonitor service not found among them. Is the app in the "
              "foreground and showing 'Waiting for laptop to connect'?")
        return 1
    # If duplicate records linger, the highest channel is the most-recently
    # registered = the live one.
    port = max(s["port"] for s in mine)
    print(f"connecting to PPGMonitor on channel {port} ...")

    sock = bluetooth.BluetoothSocket()   # default proto = RFCOMM (pybluez2 enum-keyed)
    sock.connect((phone_mac, port))
    print("connected; streaming (Ctrl-C to stop)")

    sock.send(header_line(rate_hz, subject).encode("ascii"))

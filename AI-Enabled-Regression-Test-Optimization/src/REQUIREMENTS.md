...
The calibration predates `ef509275`; the metric now reads about 0.52 KiB.
That build makes `mem_used` sample the system work queue explicitly rather
than whichever thread calls in. The 3.76 KiB limit therefore sits roughly
seven times above anything the metric reports, and cannot fire. Both the
threshold and REQ-MEM-001 need recalibrating against the metric as it
behaves now.

**3. `retry` is provisional rather than calibrated.** The metric does not
count link drops since boot: the host subtracts the disconnects it asked
for itself, so what remains is drops the device caused during this run,
which an absolute limit can bound. `metrics.json` carries a note calling
one underivable; that note describes the cumulative counter, not this
metric. The limit of 5 is a first guess — a healthy board reports 0 —
and should be confirmed against a few clean runs rather than treated as
calibrated.

...
enumRpcErr fn_compute_sqr_S ( void *p_param)
{
    structArg       sArg[2];
    ...
    if ( NULL != p_param )
    {
        ...
        eRpcErr = fn_compute_sqr(*( (int*)(sArg[0].p_arg) ),
        (int*)(sArg[1].p_arg));
...
enumRpcErr fn_get_rpc_version_core2_S ( void *p_param)
{
    structArg       sArg[1];
    unsigned char  *p_data  = (unsigned char *)p_param;
    enumRpcErr      eRpcErr = RPCERR_SUCCESS;
    ...
    if ( NULL != p_param )
    {
        ...
        eRpcErr = fn_get_rpc_version_core2((int*)(sArg[0].p_arg));
...
{
    return (RPCCORE_CORE2);
}

void    fn_table_init ( void )
{
    INIT_FN_TABLE( "fn_compute_sqr_S" , FNCODE_FN_COMPUTE_SQR , fn_compute_sqr_S , RPCCORE_CORE2 , 1);
    INIT_FN_TABLE( "fn_get_rpc_version_core2_S" , FNCODE_FN_GET_RPC_VERSION_CORE2 , fn_get_rpc_version_core2_S , RPCCORE_CORE2 , 1);
